...
::::: notes
Limitations
Scripts are often the starting point for many R users, especially in pharma where SAS programs have been the norm.
...
they have formal documentation systems, good portability dependency management, built-in testing frameworks, and proper versioning.
:::::
...
A big challenge when writing package code the design API! Basically you ar not writing code for you  nor code that can be modified by the user SO
you need to spend a lot of time thinking about different facets of implementing your code
Once we had the core functionality working, we needed to focus on: User-friendly API, Consistent interface and building clear documentation !
"Also think about - **Future extensibility** **consistent naming patterns** (because inconsistency  confuses users) example: `treatment_group` in one function but `trt_grp` in another. and consistent ones like: the tidyverse prefixing (helps reduce cognitive load and function discovery) prefix str_ map_
Also very important is - **Backward compatibility** in pharma can add a feature without breaking old ones . think about it ahead or you are cooked
:::::
...
:::: notes
As we mentioned docu is  Extremely important,
...
All that helps users understand if the package meets their regulatory requirements.!
::::
...
- **Code coverage**: Metric that measures the percentage of your package's code executed by your tests: [junco's coverage](https://johnsonandjohnson.github.io/junco/_xml_coverage_reports/coverage.html)
...
you can also add CI/CD pipelines to apply strict rules to the developers to "force" them to write minimal documentation for example
::::
...
::::: notes
Package structure provides natural integration with development tools that aren't available for scripts.
These tools save significant development time and improve code quality.
Examples include devtools for streamlined workflow, code analysis tools like lintr,
and dependency visualization with tools like pkgnet and CodeDepends.
This integration significantly reduces development time and improves code quality.
:::::

### Enhanced Development Experience

Package structure enables seamless integration with powerful tools:

::::: {.columns}

:::: {.column width="60%"}

- **devtools/usethis**: Streamlined package development workflow
- **CodeDepends**: Analyze code dependencies and structure
- **pkgnet**: Visualize package structure and dependencies
- **lintr/styler**: Enforce coding standards automatically
- **profvis**: Profile and optimize package performance
...
library(pkgnet)
CreatePackageReport("junco")
```
[`junco` package report](https://munoztd0.github.io/R_Package-Dev/junco_report.html)

:::::{.callout-note}
## Development Efficiency
These tools are designed to work with the standardized structure of R packages, providing insights and automation.
:::::
...
## What about Shiny Apps ?

:::::: notes
Shiny apps can benefit greatly from being structured as packages.
This approach provides better organization, testing capabilities, and deployment options.
The frameworks mentioned offer different levels of structure and opinions on how to organize Shiny apps.
Each has its own strengths and is suitable for different types of projects.
:::::

- **Structure**: Organized code is easier to maintain
- **Modularity**: Reuse components across apps
- **Testing**: Leverage R's testing infrastructure
- **Production-readiness**: Deployment and scaling

Framework options:

- [**golem**](https://thinkr-open.github.io/golem/): Opinionated, full app-as-package workflow (dev/ scripts, CI, tests, deployment)
- [**leprechaun**](https://leprechaun.opifex.org/#/): Lighter, code generator for simple Shiny app-packages with JS/Sass support
- [**rhino**](https://appsilon.github.io/rhino/): Pharma-oriented, uses box modules, not a package but package-like structure


:::::{.callout-note}
## Framework Selection
Choose based on complexity: golem (robust), leprechaun (lean), rhino (enterprise).
:::::
...
- Internal: for proprietary algos, company-specific workflows
...
just showing the overview of the steps
  -  But it shows why this is cool and I can do that with one line of code
...
::::::: notes
- Final output visualization (this is not exactly what our output looks like , it actually exports to rtf or docx SHOW THEM
  - but for some reason doxc doesnt work very well in html presentation
- Formatting consistent with regulatory requirements
- Comparable with traditional SAS output
- Here is a link for the catalog with all outputs (only for jnj users)
:::::::
  -
...
::::::: notes
- Key Takeaways are that R packages provide a **structured framework** for organizing code and
- They **improve reproducibility** and **reduce human errors**
- They also facilitate **Documentation and testing**
- Finally they can **streamline workflows** in pharmaceutical research and improve efficiency
:::::::
